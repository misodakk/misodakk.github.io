...
x86 机器副作用有概率是有的插件的升级功能可能会失效，或者保存配置后重启丢失。
### combined (镜像类型)
...
关于镜像的选择，PVE 直接选 COMBINED-EFI (EXT4-COMBINED-EFI.IMG.GZ) 版本就行。
...
### 升级
以我使用的 ImmortalWrt 为例，由于想安装 Nikki，发现版本太低，所以需要升级，于是萌生了升级的想法，于是噩梦开始了。

操作之前先一定要备份，这个我是知道的，就结果而说，备份不能说没用，但基本是没啥用。

如果是 PVE 我这种，其实可以直接去下一个最新的镜像，直接导入进去；

自带的有个值守式系统更新；它会请求一个包含当前软件包的新固件并进行刷写。只需勾选 “保留当前配置”，无需任何额外配置即可直接升级到新固件。

这种方式虽然简单，但是可能存在一些 bug，我是 23.05.4 版本，想要升 24.x，但是搜索到的只有 23.x，无奈，就不用这个了。

最后我直接在官方下了新的 img.gz 包，通过 WebUI 的刷写操作直接刷入，选择保留当前配置。

升级完后正常进入，可能是因为大版本升级，或者重新刷写的原因，主题和插件都丢了，更悲催的是网络不通，ping ip 可以，但是域名不行，应该是 DNS 有问题，折腾了好一会都不行，最终成功的搞的彻底进不去了（应该是瞎改了防火墙配置）。。。。

使用 pve 还原后重新升级了一次，这次直接选择不保留配置，避免不兼容问题，就是需要重新修改下 IP 要不然连不上，本来也没几个插件就不走还原配置了，重新配一下；

重新安装 luci-i18n-ddns-go-zh-cn、argon、ttyd 基础插件，以及魔法上网的插件，基本就算完事了。

然后坏消息，重启后配置都丢了，可能是和选择 SquashFS 有关系，但是之前也是 SquashFS 没啥问题，最后换 EXT4 的包重新刷吧，反正 SquashFS 的恢复基本也用不到，有 PVE 的就行了。

### 旁路由
这一次经过充分调研，我决定搞一个尽量干净的环境，我的需求仅仅是保证局域网的一台 Ubuntu 服务器和 docker 可以魔法上网，其他的设备稳定才是第一优先级，所以旁路由适合我。

旁路由的配置我总结了下：
1. 网络-接口：这里只保留 lan，设备选 br-lan，配置静态 IP，网关、DNS 填主路由，DHCP 忽略，避免冲突；IPV6 没有需求全部禁用。
2. 网络-设备：这里 br-lan 里只选一个你在用的网口，有 WiFi 的话也选上，否则就是一个，禁用 IPV6;
   实际的网口 eth0 把 IPV6 关掉，其他的不要动。
3. 网络-DHCP/DNS：取消唯一授权，其他都不要动。
4. 网络-防火墙：取消 SYN-flood 防御，不要选丢弃无效数据包，其他的入站出站转发默认可以全接受，反正也不上外网

至此，基本就配置完了，只要把终端设备的网关和 DNS 改成这个旁路由就行了，DNS 也要改，折腾了好久，哎。。

目前我只装了个 ddns-go 和 nikki / momo，其他的都没装；
passwall2 实在是用不明白，openClash 感觉有点乱，还是 Nikki 简洁。

修改 DNS 先看看 `ls -l /etc/resolv.conf` 具体指向那，改这个具体指向的文件。
